...
- SKD009 ("model worse than baseline") now always includes the baseline's performance, and is reported as a tip rather than an issue. It only becomes an issue if the model is much worse than the baseline, instead of when the model is not significantly better. One consequence of this change is that using the baseline itself as a model no longer triggers SKD009.
- SKD010 ("model slower than baseline") class now compares both fit time and predict time, not just fit time, so a model that is slow to predict is caught as well.
