...
class AuthenticatedClient(Client):
    ...
    def authorization(self) -> str:
        """
        Authorization based on API key or token, depending on the context.

        Notes
        -----
        The token is not persisted in RAM to ensure logout: it must be available on disk
        before any request to the ``skore hub``.
        """
        if (apikey := environ.get("SKORE_HUB_API_KEY")) is not None:
            return f"X-API-Key: {apikey}"
        else:
            token = Token()
    ...
    def request(
        ...
        **kwargs,
        ...
        # Overload headers with our authorization (API key or token)
