...
import type { EstimatorReportPlot } from "@/components/EstimatorReport.vue";
import PlotlyWidget from "@/components/PlotlyWidget.vue";

const props = defineProps<{ plots: EstimatorReportPlot[] }>();
const currentPlotIndex = ref(0);
const currentPlot = computed<EstimatorReportPlot>(() => {
  return props.plots[currentPlotIndex.value];
});
</script>

<template>
  <div class="estimator-report-plots" v-if="currentPlot">
    <div class="header">
