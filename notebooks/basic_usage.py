...
# %% [markdown]
# Now that you have created the `project.skore` folder (even though nothing has yet been stored), you can run the dashboard (in your project root i.e. where `project.skore` is):
# ```python3
...
# %% [markdown]
# By using the `delete_item` method, you can also delete an object so that your `skore` dashboard does not become cluttered:
...
# %% [markdown]
# Note that the media type is only used for the dashboard, and not in this notebook at hand:
