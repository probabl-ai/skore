...
def test_open_relative_path(fake_project_path):
    """If passed a relative path, `open` operates in the current working directory."""
    os.chdir(fake_project_path.parent)
    p = open(fake_project_path.name, create=False)
    assert isinstance(p, Project)


def test_open_default(fake_project_path):
    """If a project already exists, `open` loads it."""
    with project_changed(fake_project_path, modified=False):
        p = open(fake_project_path)
        assert isinstance(p, Project)
...
def test_open_project_exists_create_true_overwrite_true(fake_project_path):
    with project_changed(fake_project_path):
        p = open(fake_project_path, create=True, overwrite=True)
        assert isinstance(p, Project)


def test_open_project_exists_create_true_overwrite_false(fake_project_path):
    with pytest.raises(FileExistsError):
        open(fake_project_path, create=True, overwrite=False)


def test_open_project_exists_create_false_overwrite_true(fake_project_path):
    p = open(fake_project_path, create=False, overwrite=True)
    assert isinstance(p, Project)


def test_open_project_exists_create_false_overwrite_false(fake_project_path):
    p = open(fake_project_path, create=False, overwrite=False)
    assert isinstance(p, Project)
