...
def test_normalization(forest_binary_classification_data):
    ...
    cv_report_2 = CrossValidationReport(estimator, X, y, splitter=cv)
    ...
    for estimator_name in report.reports_:
        for split_idx in range(cv):
            cm_true = (
                display.frame(threshold_value=threshold_val, normalize="true")
            ...
            cm_pred = (
                display.frame(threshold_value=threshold_val, normalize="pred")
                .query(f"estimator == '{estimator_name}'")
                .query(f"split == {split_idx}")
            )
            ...
            cm_all = (
                display.frame(threshold_value=threshold_val, normalize="all")
                .query(f"estimator == '{estimator_name}'")
                .query(f"split == {split_idx}")
            )
...
def test_threshold_greater_than_max(
    ...
    for estimator_name in frame["estimator"].unique():
        for split_idx in frame["split"].unique():
            split_frame = frame.query(
                f"estimator == '{estimator_name}' and split == {split_idx}"
            )
