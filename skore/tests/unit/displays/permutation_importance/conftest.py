...
@pytest.fixture(scope="module")
def regression_half(regression):
    X, y = regression
    return X[:, :2], y


@pytest.fixture(scope="module")
def regression_train_test_split_half(regression_half):
    X, y = regression_half
    return train_test_split(X, y, test_size=0.2, random_state=0)


@pytest.fixture(scope="module")
def comparison_estimator_reports_regression_different_features(
    estimator_reports_regression, regression_train_test_split_half
):
    X_train, X_test, y_train, y_test = regression_train_test_split_half
    report_1 = estimator_reports_regression[0]
    report_2 = EstimatorReport(
        LinearRegression(),
        X_train=X_train,
        y_train=y_train,
...
def comparison_cross_validation_reports_regression_different_features(
    cross_validation_reports_regression, regression_half
):
    X, y = regression_half
    report_1 = cross_validation_reports_regression[0]
    report_2 = CrossValidationReport(LinearRegression(), X=X, y=y, splitter=2)
    return ComparisonReport([report_1, report_2])


@pytest.fixture(scope="module")
def multioutput_regression_half(multioutput_regression):
    X, y = multioutput_regression
    return X[:, :2], y


@pytest.fixture(scope="module")
def multioutput_regression_train_test_split_half(multioutput_regression_half):
    X, y = multioutput_regression_half
...
@pytest.fixture(scope="module")
def comparison_estimator_reports_multioutput_regression_different_features(
    estimator_reports_multioutput_regression,
    multioutput_regression_train_test_split_half,
):
    X_train, X_test, y_train, y_test = multioutput_regression_train_test_split_half
    report_1 = estimator_reports_multioutput_regression[0]
    report_2 = EstimatorReport(
        LinearRegression(),
        X_train=X_train,
        y_train=y_train,
...
@pytest.fixture(scope="module")
def comparison_cross_validation_reports_multioutput_regression_different_features(
    cross_validation_reports_multioutput_regression, multioutput_regression_half
):
    X, y = multioutput_regression_half
    report_1 = cross_validation_reports_multioutput_regression[0]
    report_2 = CrossValidationReport(LinearRegression(), X=X, y=y, splitter=2)
    return ComparisonReport([report_1, report_2])
