...
def test_binary_classification(
    ...
    result = display.plot(subplot_by="estimator")
    assert isinstance(result.axes.flatten(), np.ndarray)
    assert len(result.axes.flatten()) == len(report.reports_)
    for report_name, ax in zip(report.reports_, result.axes.flatten(), strict=True):  # noqa: B007
        # report_name is used in the pandas.query as an string which is not detected by
        # ruff as a used variable
...
def test_multiclass_classification(
    ...
    assert result.figure.get_suptitle() == "Coefficients by estimator"
...
def test_different_features(
    ...
    for subplot_by in ("auto", "estimator"):
        result = display.plot(subplot_by=subplot_by)
        assert isinstance(result.axes.flatten(), np.ndarray)
        assert len(result.axes.flatten()) == len(report.reports_)
        for report_name, ax in zip(report.reports_, result.axes.flatten(), strict=True):
            assert isinstance(ax, mpl.axes.Axes)
            assert ax.get_title() == f"estimator = {report_name}"
            assert ax.get_xlabel() == "Magnitude of coefficient"
            assert ax.get_ylabel() == ""
...
def test_include_intercept(
    ...
    assert result.figure.get_suptitle() == "Coefficients"
