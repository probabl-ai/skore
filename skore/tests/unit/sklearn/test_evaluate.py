...
def test_evaluate_float_splitter(regression_Xy):
    """A float splitter triggers a train-test split and returns an EstimatorReport."""
    X, y = regression_Xy
...
def test_evaluate_multiple_estimators_prefit(regression_Xy):
    """A list of fitted estimators with splitter='prefit' returns a ComparisonReport."""
    X, y = regression_Xy
...
def test_evaluate_multiple_estimators_cv(regression_Xy):
    """A list of estimators with int splitter returns a ComparisonReport of CV."""
    X, y = regression_Xy
    estimators = [LinearRegression(), LinearRegression()]
...
def test_evaluate_invalid_splitter_type(regression_Xy):
    """A splitter of unsupported type raises TypeError."""
    X, y = regression_Xy
    with pytest.raises(TypeError, match="Invalid type for splitter"):
        evaluate(LinearRegression(), X, y, splitter=[1, 2, 3])
