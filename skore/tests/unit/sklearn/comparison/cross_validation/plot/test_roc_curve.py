...
def test_multiclass_classification_pos_label(pyplot):
    """Check the behaviour of `roc_curve` when ML task is "multiclass-classification"
    and `pos_label` is not None.

    The plot should be similar to the binary classification case.
    """
    ...
    display = report.metrics.roc(pos_label=0)
    assert isinstance(display, RocCurveDisplay)
    ...
    assert (
        display.ax_.get_ylabel() == f"True Positive Rate\n(Positive label: {pos_label})"
    )
    assert display.ax_.get_adjustable() == "box"
    assert display.ax_.get_aspect() in ("equal", 1.0)
    assert display.ax_.get_xlim() == display.ax_.get_ylim() == (-0.01, 1.01)
