...
def test_coefficients_display_top_k_correct_selection(regression_train_test_split):
    """Check that top_k correctly selects features with largest absolute
    coefficients."""
    X_train, X_test, y_train, y_test = regression_train_test_split
    report = EstimatorReport(
    ...
    # Calculate expected top k features based on absolute coefficients
    abs_coefs = full_frame["coefficients"].abs()
    expected_features = set(
        full_frame.loc[abs_coefs.nlargest(top_k).index, "feature"].tolist()
    )
...
def test_coefficients_display_bottom_k_correct_selection(regression_train_test_split):
    """Check that bottom_k correctly selects features with smallest absolute
    coefficients."""
    X_train, X_test, y_train, y_test = regression_train_test_split
    report = EstimatorReport(
        Ridge(), X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test
    ...
    # Calculate expected bottom k features based on absolute coefficients
    abs_coefs = full_frame["coefficients"].abs()
    expected_features = set(
