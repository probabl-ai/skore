    ...
    def _plot_single_estimator(
        ...
        *,
        ...
        if self.ml_task == "binary-classification":
            ...
            precision_recall = _filter_by(self.precision_recall, label=pos_label)
            ...
            for class_idx, class_label in enumerate(labels):
                precision_recall = _filter_by(self.precision_recall, label=class_label)
                average_precision = _filter_by(
                    self.average_precision, label=class_label
        ...
        *,
        ...
        if self.ml_task == "binary-classification":
            ...
            for split_idx in self.precision_recall["split_index"].unique():
                precision_recall = _filter_by(
                    self.precision_recall, label=pos_label, split_index=split_idx
                )
                average_precision = _filter_by(
                    self.average_precision, label=pos_label, split_index=split_idx
                )["average_precision"].iloc[0]
            ...
            for class_idx, class_label in enumerate(labels):
                ...
                for split_idx in self.precision_recall["split_index"].unique():
                    precision_recall = _filter_by(
                        self.precision_recall, label=class_label, split_index=split_idx
                    )
                    average_precision = _filter_by(
                        self.average_precision, label=class_label, split_index=split_idx
                    )["average_precision"]
        ...
        *,
        ...
        if self.ml_task == "binary-classification":
            ...
            for est_idx, est_name in enumerate(estimator_names):
                precision_recall = _filter_by(
                    self.precision_recall, label=pos_label, estimator_name=est_name
                )
                average_precision = _filter_by(
                    self.average_precision, label=pos_label, estimator_name=est_name
                )["average_precision"].iloc[0]
        ...
        *,
        ...
        if self.ml_task == "binary-classification":
            ...
            for report_idx, estimator_name in enumerate(estimator_names):
                average_precision = _filter_by(
                    self.average_precision, estimator_name=estimator_name
                )["average_precision"]
