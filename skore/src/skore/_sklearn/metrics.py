...
def make_metric(
    ...
    *,
    name: str | None = None,
    response_method: str | list[str] | tuple[str, ...] | None = "predict",
    ...
    metric : str, callable, sklearn scorer, or Metric
        ...
        - If a string, will be converted via :func:`sklearn.metrics.get_scorer`.
        ...
        - If a callable, expected to be of the form ``(y_true, y_pred, **kw) -> float``.
        - If a sklearn scorer, expected to be a _BaseScorer instance (e.g. as returned
          by :func:`sklearn.metrics.make_scorer` and
    ...
    if isinstance(metric, Metric):
        ...
        # if metric == "f1", user should be able to ask for "f1", not "f1_score"
        name = name if name is not None else metric.removeprefix("neg_")
        return make_metric(scorer, name=name)
    elif callable(metric):
        if response_method is None:
            raise ValueError(
                "response_method is required when metric is a plain callable. "
                "Pass it directly or use sklearn.metrics.make_scorer to create "
                "a scorer with a response_method."
            )
        ...
        if missing_kwargs:
            ...
            raise TypeError(
                f"Callable {_callable_name(metric)!r} has required parameter(s) "
                f"{missing_kwargs} not covered by the provided kwargs. "
                f"Pass them as keyword arguments: "
...
class Metric:
    ...
    For custom (user-added) metrics, instantiate directly.
