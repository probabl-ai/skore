...
def _truncate_top_k_categories(
    ...
    keep = sbd.is_in(col, values) | sbd.is_null(col)  # type: ignore[assignment]
...
class TableReportDisplay(ReprHTMLMixin, DisplayMixin):
    ...
    def _plot_distribution_1d(
        ...
        *,
        ...
        if is_categorical := not (sbd.is_numeric(column) or sbd.is_any_date(column)):
            ...
            column = sbd.filter(column, sbd.is_in(column, top_k))  # type: ignore[assignment]
    ...
    def _plot_distribution_2d(
        ...
        *,
        ...
        hue = _truncate_top_k_categories(hue, k)  # type: ignore[assignment]
        ...
        if is_x_num and is_y_num:
            ...
            if is_x_num:
                y = _truncate_top_k_categories(y, k)  # type: ignore[assignment]
            else:
                x = _truncate_top_k_categories(x, k)  # type: ignore[assignment]
